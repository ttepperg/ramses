...
subroutine update_sink(ilevel)
  ...
  real(dp),allocatable,dimension(:),save::msink_save
  ...
  do isink=1,nsink
     if(msink(isink)>0.0)then
        ...
        if (first_call)then
           ...
           msink_save(:) = 0.
           msink_save(isink) = msink(isink)
           first_call=.false.
        endif
        if (remove_sink)then
           if (t>sink_tscale0)then
              msink(isink)=msink_save(isink)*exp((sink_tscale0-t)/sink_tscale)
              if ((myid==1).and.print_when_io)then
                 write(*,*)'Decreasing mass of sink ',idsink(isink),&
                 &' from its initial mass ',msink_save(isink),&
                 &'to ',msink(isink), 'at level ',ilevel
              endif
           endif
        endif
...
!     call clean_stop
  ...
  ! special settings, require a lot of care and are not compatible with
