...
104  continue
     ...
     ! Loading sinks from the ICs (ic_sink)
     nsink=0
     nindsink=0
     nsinkold=0
     if(TRIM(initfile(levelmin)).NE.' ')then
  ...
  if (ic_sink .or. ic_sink_restart)then
     ...
     do
        ...
        nindsink=nindsink+1
        idsink(nsink)=sid
        msink(nsink)=sm1*M_sun/scale_m
        xsink(nsink,1)=sx1*kpc2cm/scale_l+boxlen/2
        xsink(nsink,2)=sx2*kpc2cm/scale_l+boxlen/2
        xsink(nsink,3)=sx3*kpc2cm/scale_l+boxlen/2
        vsink(nsink,1)=sv1*km2cm/scale_v
        vsink(nsink,2)=sv2*km2cm/scale_v
        vsink(nsink,3)=sv3*km2cm/scale_v
        lsink(nsink,1)=sl1*M_sun*kpc2cm*km2cm/scale_m/scale_l/scale_v
        lsink(nsink,2)=sl2*M_sun*kpc2cm*km2cm/scale_m/scale_l/scale_v
        lsink(nsink,3)=sl3*M_sun*kpc2cm*km2cm/scale_m/scale_l/scale_v
        tsink(nsink)=t
