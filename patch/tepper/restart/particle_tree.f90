...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  if(restart_init) then
     do ivar=1,nvar_min
        do i=1,np
#ifdef LIGHT_MPI_COMM
           reception(icpu,ilevel)%pcomm%u(current_property,ind_com(i))=varp(ind_part(i),ivar)
#else
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  if(restart_init) then
     do ivar=1,nvar_min
        do i=1,np
#ifdef LIGHT_MPI_COMM
           varp(ind_part(i),ivar)=emission_part(ilevel)%u(current_property, offset_np+ind_com(i)-1)
#else
